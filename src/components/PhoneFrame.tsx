...
export function PhoneScreenshot({
  ...
  return (
    <div className={clsx('relative aspect-366/729', className)}>
      <div className="absolute inset-y-[calc(1/729*100%)] right-[calc(5/729*100%)] left-[calc(7/729*100%)] rounded-[calc(58/366*100%)/calc(58/729*100%)] shadow-2xl" />
      <div className="absolute top-[calc(23/729*100%)] left-[calc(23/366*100%)] h-[calc(686/729*100%)] w-[calc(318/366*100%)] overflow-hidden rounded-[calc(48/366*100%)/calc(48/729*100%)] bg-gray-900">
        <Image
          src={src}
          alt={alt}
          className="h-full w-full object-cover"
          sizes="(min-width: 1024px) 366px, 60vw"
          priority={priority}
        />
      </div>
      <PlaceholderFrame className="pointer-events-none absolute inset-0 h-full w-full fill-transparent" />
      <Image
        src={frame}
        alt=""
        className="pointer-events-none absolute inset-0 h-full w-full"
